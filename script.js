...
function init() {
    ...
    // Platform pertama agar posisi pas di depan stickman
    const firstWidth = 80;
...
// LOGIKA: Lebar kotak semakin mengecil seiring naiknya skor
...
// --- INPUT CONTROL (INSTANT HOLD OPTIMIZED) ---
function startAction(e) {
    if (e && e.cancelable) e.preventDefault(); // Mencegah interupsi browser/zoom
...
// Menggunakan pointerdown agar sekali sentuh langsung hold (tanpa delay 300ms)
actionBtn.addEventListener("pointerdown", startAction, { passive: false });
window.addEventListener("pointerup", endAction);
// Mencegah menu klik kanan/tahan lama muncul
...
function update() {
    ...
    if (gameState === "TURNING") {
        ...
        // Logika Jatuh: Putar 180 derajat ke bawah jika tidak sampai
